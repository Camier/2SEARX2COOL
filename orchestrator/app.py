#!/usr/bin/env python3
"""
2SEARX2COOL Production Orchestrator
...
def create_app():
    ...
    with app.app_context():
        ...
        try:
            from blueprints.api.public_routes import public_api_bp
            app.register_blueprint(public_api_bp, url_prefix='/public')
            blueprints_loaded['public_api'] = True
            logger.info("✅ Public API blueprint registered (no auth required)")
        except Exception as e:
            logger.warning(f"⚠️ Public API blueprint failed: {e}")
...
def register_routes(app):
    ...
    def index():
        ...
        return jsonify({
            'status': 'healthy',
            'service': '2searx2cool-orchestrator',
            'version': '1.0.0',
            'mode': 'production',
            'features': {
                'websocket': True,
